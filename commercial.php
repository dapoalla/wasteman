<?php
$title = "Commercial Customers (KONGI)";
require_once "header.php";
require_once "db.php";

// Redirect if not KONGI user
if ($_SESSION['subsidiary'] !== 'KONGI') {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['customer_id'])) {
    ...
    if ($_SESSION['role'] == 'admin') {
        if ($action == 'delete') {
            $stmt = $conn->prepare("DELETE FROM commercial_customers WHERE id = ?");
            $stmt->bind_param("i", $customer_id);
             if ($stmt->execute()) { $message = "Customer record deleted."; $message_type = 'success'; }
        } elseif ($action == 'toggle_high_value') {
            $stmt = $conn->prepare("UPDATE commercial_customers SET is_high_value = NOT is_high_value WHERE id = ?");
            $stmt->bind_param("i", $customer_id);
            $stmt->execute();
        }
    }
    ...
    if (isset($_POST['status'])) {
        $status = $_POST['status'];
        $stmt = $conn->prepare("UPDATE commercial_customers SET bill_status = ? WHERE id = ?");
        $stmt->bind_param("si", $status, $customer_id);
        if ($stmt->execute()) { $message = "Bill status updated."; $message_type = 'success'; }
    }
}

$result = $conn->query("SELECT * FROM commercial_customers ORDER BY id DESC");
...
<div class="container mx-auto">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-3xl font-bold text-white">Commercial Customers</h2>
         <div>
            <a href="import.php" class="bg-amber-600 hover:bg-amber-700 text-white font-bold py-2 px-4 rounded-lg inline-flex items-center"><span class="material-symbols-outlined mr-2">upload</span> Import</a>
            <?php if ($_SESSION['role'] == 'admin'): ?>
            <a href="add_edit_customer.php?type=commercial" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded-lg ml-4 inline-flex items-center"><span class="material-symbols-outlined mr-2">add</span> Add Customer</a>
            <?php endif; ?>
        </div>
    </div>
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-700 hover:bg-gray-700/40">
                        <td class="px-6 py-4 font-medium text-white whitespace-nowrap">
                             <a href="view_customer.php?type=commercial&id=<?php echo $row['id']; ?>" class="hover:underline <?php echo $theme_color_class; ?>">
                        ...
                        <td class="px-6 py-4 font-bold text-amber-400">₦<?php echo number_format($row['balance'], 2); ?></td>
                        ...
                        <td class="px-6 py-4 flex items-center gap-1">
                            ...
                            <?php if ($_SESSION['role'] == 'admin'): ?>
                                <a href="add_edit_customer.php?type=commercial&id=<?php echo $row['id']; ?>" title="Edit" class="p-2 text-gray-400 hover:text-white hover:bg-gray-500/30 rounded-full"><span class="material-symbols-outlined">edit</span></a>
                    ...
                    <?php endwhile; else: ?>
                    <tr><td colspan="4" class="text-center py-8">No commercial customer data found.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
